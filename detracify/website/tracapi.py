...
SERVER_URL = "https://%s:%s@code.djangoproject.com/login/rpc"
DESCRIPTION_STRING = "Posted: %a, %d %b %Y %H:%M:%S +0000"
...
class TicketAPI(object):
    def __init__(self, username, passwd):
        self.server_url = SERVER_URL % (username, passwd)
    ...
    def put_attachment(self, ticket_id, data, filename=None, description=None):
        ...
        if description is None:
            description = time.strftime(DESCRIPTION_STRING, ts)

        if self.get_ticket(ticket_id):
            b64_data = xmlrpclib.Binary(data)
            return self.server_proxy.ticket.putAttachment(ticket_id, filename,
                    description, b64_data, False)
    ...
    def get_changelog(self, ticket_id, ts=None):
        ...
        changes equal to or after date time.  Pretty sure date time should be
        UTC but don't quote me on that.  @pizzapanther
        """
